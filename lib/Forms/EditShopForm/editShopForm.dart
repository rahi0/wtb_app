...
class EditShopForm extends StatefulWidget {
  ...
  EditShopForm(this.shopID);
  @override
  State<StatefulWidget> createState() {
    return EditShopFormState();
  }
}

class EditShopFormState extends State<EditShopForm> {
  var shopLink;
  bool loading = false;
  var productList;
  ...
  String theme = "", user = "", logoImg, bannerImg;
  String typeService = "",
      productBuyer = "",
      productSeller = "",
      productCategory = "",
  ...
  Future loadCategories() async {
    ...
    setState(() {
      catList = data;
      catLength = catList.interests.length;
    });
  }
  ...
  Future loadData() async {
    ...
    shopNameController.text = productList.shopDetails.shopName;
    shopLinkController.text = productList.shopDetails.shopLink;
    shopLocationController.text = productList.shopDetails.location;
    shopTagLineController.text = productList.shopDetails.shopTagLine;
    shopDescriptionController.text = productList.shopDetails.about;
    String type = productList.shopDetails.shopType;
    if (type == "product") {
      _dropDownCategoryItems = getDropDownSizeCats();
      productCategory = _dropDownCategoryItems[0].value;
    } else {
      _dropDownCategoryItems = getDropDownSizeCats();
      productCategory = _dropDownCategoryItems[1].value;
    }
    int id = productList.shopDetails.shopcategoryId;

    for (int i = 0; i < catLength; i++) {
      if (catList.interests[i].id == id) {
        selectedCategory.add("${catList.interests[i].name}");
        selectedCat = selectedCategory.toList();
        isOpen = false;
        selectedID = catList.interests[i].id;
        cat = catList.interests[i].name;
      }
    }
    status = productList.shopDetails.status;
    sellerID = productList.shopDetails.sellerId;
    created = productList.shopDetails.createdAt;
    updated = productList.shopDetails.updatedAt;
...
////////
                              ...
                              child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                      ...
                                      borderRadius: BorderRadius.all(
                                          Radius.circular(15))),
                                  child: Text(
                                    isSubmit ? "Please wait..." : "Edit Shop",
                                    style: TextStyle(
                                      color: isSubmit
                                          ? Colors.black
                                          : Colors.white,
  ...
  void signUpButton() async {
    ...
    var res = await CallApi().postData1(data2, 'edit/Shop/${widget.shopID}');
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Container(
                        ...
                        child: Text(
                          "Shop has been edited successfully!",
                          textAlign: TextAlign.start,
                          style: TextStyle(
                              color: Colors.black54,
                              fontSize: 14,
                            ...
                            onTap: () {
                              setState(() {
                                ...
                                Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                        builder: (context) => ShopPage(widget.shopID)));
                              });
                            },
                            child: Container(
                                padding: EdgeInsets.all(10),
