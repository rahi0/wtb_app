...
class EditPostForm extends StatefulWidget {
  ...
  EditPostForm(this.postList, this.userData, this.index);
  @override
...
class EditPostFormState extends State<EditPostForm> {
  ...
  void initState() {
    ...
    interests = widget.postList.res[widget.index].interest;
    postController.text = widget.postList.res[widget.index].status;
    status = widget.postList.res[widget.index].privacy;
    for (int j = 0; j < widget.postList.res[widget.index].images.length; j++) {
      statusPic = widget.postList.res[widget.index].images[j].thum;
      //print(statusPic);
      int id = widget.postList.res[widget.index].images[j].id;
      print(id);
      if (statusPic.contains("localhost")) {
        statusPic = statusPic.replaceAll("localhost", "http://10.0.2.2");
        allImages.add(id);
        images.add(statusPic);
      } else {
        allImages.add(id);
        images.add(statusPic);
      }
                    ...
                    return GestureDetector(
                        onTap: () {
                          setState(() {
                            ...
                            widget.postList.res[widget.index].images
                                .removeAt(index);
                            print(images.length);
                          });
                        },
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                              ...
                              image: DecorationImage(
                                  fit: BoxFit.cover,
                                  image: img[index].contains("storage")
                                      ? FileImage(File(img[index].toString()))
                                      : NetworkImage(img[index].toString()))),
  ...
  Future statusUpload() async {
    ...
    var data = {
      'id': widget.postList.res[widget.index].id,
      'interest': interests,
      'isEdit': true,
      'link': null,
      'link_meta': null,
      ...
      'images': widget.postList.res[widget.index].images
    };
